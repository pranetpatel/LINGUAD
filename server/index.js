...
app.get("/api/config", (_, res) => ok(res, { ai: !!ANTHROPIC_KEY, tts: !!ELEVEN_KEY, stt: !!ELEVEN_KEY, streamingAsr: streamingAvailable() }));
...
/* ── server-held AI key: Anthropic proxy ── */
app.post("/api/ai", requireAuth, aiLimit.mw(byAccount), async (req, res) => {
  try {
    if (!ANTHROPIC_KEY) return fail(res, { status: 503, message: "Server has no ANTHROPIC_API_KEY configured" });
...
server.listen(PORT, () => console.log(`Lingua server on :${PORT}  ai=${!!ANTHROPIC_KEY} tts/stt=${!!ELEVEN_KEY} streamingAsr=${streamingAvailable()}`));
